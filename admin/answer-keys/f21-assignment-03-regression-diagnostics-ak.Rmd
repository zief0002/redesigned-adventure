...
```{r, message=FALSE, fig.height=4, fig.width=8, out.width='6in', fig.align='center'}
#scatterplotMatrix(stack[-1], smooth = FALSE)[1, ]
p1 = ggplot(data = stack, aes(x = turnout, y = inequality)) +
...
# Mean-shift model
lm_ms = lm(inequality ~ 1 + turnout + energy + d_argentina, data = stack)
tidy(lm_ms)
...
$$
\hat{\mathrm{Inequality}}_i = 9.24 - 0.071(\mathrm{Voter~Turnout}_i) - 0.0002(\mathrm{Energy}_i)
$$
