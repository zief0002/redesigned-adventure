---
title: "Assignment 01"
subtitle: "Matrix Algebra for Linear Regression"
format:
  html:
...
5. Use the values from **b** (Question 3) and from the variance--covariance matrix you reported in the previous question to find the 95% CI for the coefficient associated with the main-effect of PCI. (Hint: If you need to refresh yourself on how CIs are computed, see [here](https://zief0002.github.io/gentrified-ladybug/notes/s22-05-probability-distributions-in-regression.html).)
...
13. Use the mean square terms you found in Question 13 to compute the *F*-value for the model (i.e., to test $H_0:\rho^2=0$). Also compute the *p*-value associated with this *F*-value. (Hint: If you need to refresh yourself on how *F*-values or *p*-values are computed, see [here](https://zief0002.github.io/gentrified-ladybug/notes/s22-05-probability-distributions-in-regression.html).)
...
Now consider fitting the model to the data to examine whether there is an effect of region (no other predictors) on infant mortality. In this model, we will use effects coding to encode the region variable (see [here](https://stats.idre.ucla.edu/other/mult-pkg/faq/general/faqwhat-is-effect-coding/)). In effects coding, rather than using 0 and 1 (dummy coding), we use +1 and $-1$ to code the different categories.
